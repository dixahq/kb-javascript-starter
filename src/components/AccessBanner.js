...
function AccessBanner() {
  ...
  return (
    <div className="access-banner">
      <div className="access-banner-header" data-testid="article-access">
        ...
        <div className="access-banner-title">
          {access === AccessLevel.Restricted ? (
            <Trans i18nKey="internal.restricted">For internal use, with restrictions</Trans>
          ) : (
            <Trans i18nKey="internal.unrestricted">For internal use</Trans>
          )}
        </div>
        {showRestrictions && (
          <button className="access-banner-toggle" onClick={() => setShowing(!showing)} type="button">
            {showing ? (
              <svg width="24" height="24" viewBox="0 0 24 24" className="access-banner-icon">
      ...
      {showRestrictions && showing && (
        <ul className="access-banner-body">
          {smartGroups.map((s, i) => (
            <li key={i} className="access-banner-group">
              {s.name}
            </li>
          ))}
        </ul>
