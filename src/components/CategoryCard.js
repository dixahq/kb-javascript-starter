...
function CategoryCard({ categoryId, className }) {
  ...
  return (
    <Category.Link className={cx('card', className)} data-testid="category-link">
      ...
      <p className="card-info">
        <Meta.If test={subCategoriesCount > 0}>
          <Trans i18nKey="categorycard.categoriesCount" count={subCategoriesCount}>
            {{
        ...
        <Meta.If test={subCategoriesCount > 0 && !!totalCount && totalCount > 0}> &amp; </Meta.If>
